...
#
# --no-quarantine is not optional advice. Homebrew attaches the quarantine
# attribute like any other download, and nib is not notarised, so without the
# flag Gatekeeper refuses to open it and offers only "Move to Trash". This cask
# cannot strip the flag itself: doing it in a postflight would defeat Gatekeeper
# silently, which is not a decision an install script should make for someone.
cask "nib" do
  version "0.1.0"
  ...
  caveats <<~EOS
    ...
    Or reinstall with: brew install --cask --no-quarantine ...
